...
(() => {
    ...
    Game.enemyDefeated=async function(){const e=this.state.enemy,p=this.state.player;if(!e)return;const summary=this.state.battleSummary||(this.state.battleSummary={xp:0,gold:0,defeated:[],drops:[],levels:0,location:this.state.location});summary.xp+=e.xp||0;summary.gold+=e.gold||0;summary.defeated.push(e.name);p.xp+=e.xp||0;p.gold+=e.gold||0;this.state.kills++;this.addNarrative(`${e.name} defeated.`,'combat');this.checkQuests('kill',e.name);this.restoreMonsterDebuffs?.();
        ...
        // v7.17.2 FIX: level-up state is applied FIRST so the professional
        // timeline (defeat sound -> loot -> experience -> victory fanfare) can
        // narrate the TRUE final level. Sound order is now always:
        // finalHit -> death -> bodyFall -> loot/treasure -> XP -> levelup ->
        // [summary text] -> victory fanfare LAST.
        const add=this.addNarrative;this.addNarrative=()=>{};const oldLevel=p.level;while(p.xp>=p.xpToNext){this.levelUp();summary.levels++;}this.addNarrative=add;
        const pro=window.ProfessionalAudioCombat;
        if(pro){
          await pro.monsterDefeat(e.name);
        ...
        add.call(this,`Battle finished. ${defeatedGroup} defeated.`,'treasure');
