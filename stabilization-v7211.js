...
(() => {
    ...
    const SpellSystem={
        config:Object.freeze({healing:Object.freeze({cost:14,cooldownMs:6000}),multipleStrike:Object.freeze({cost:18,cooldownMs:8000,failureChance:.12,maxTargets:3}),openingDoors:Object.freeze({cost:8,cooldownMs:3000})}),
        random:()=>Math.random(),
        castHealing(game){
            ...
            if(p.hp>=p.maxHp){game.addNarrative('You are already at full health.','system');return false;}
            ...
            if(this.random()<failure){setCooldown(game,'healing',Math.floor(this.config.healing.cooldownMs/2));recordMastery(game,spell,{mp:cost});event(game,`${spell} falters after using ${cost} mana.`,'magic');game.updateHUD();game.save();if(game.state.inCombat)game.enemyAttack();return false;}
        ...
        castMultipleStrike(game){
            ...
            if(this.random()<this.config.multipleStrike.failureChance){recordMastery(game,spell,{mp:cost});event(game,`Multiple Strike fails after using ${cost} mana.`,'magic');game.updateHUD();game.save();game.enemyGroupTurn?.();return false;}
            ...
            recordMastery(game,spell,{mp:cost,damage:total});MusicSystem.playSFX('magic');const text=hits?`You cast Multiple Strike. ${numberWords[hits]||hits} ${hits===1?'enemy is':'enemies are'} struck for ${total} total damage.`:'You cast Multiple Strike, but every target blocks it.';event(game,text,'magic',{eventId:`multi:${Date.now()}`});game.updateHUD();game.updateEnemyHUD();game.updateEnemyGroupHUD?.();game.save();if(game.state.enemy.hp<=0)game.enemyDefeated();else game.enemyGroupTurn?.();return true;
        },
    ...
    const applyGamePatches=()=>{
        ...
        Game.beginCommandCombat=function(description){this.closePanels();const status=document.getElementById('combat-status'),text=document.getElementById('combat-status-text');if(status)status.classList.remove('hidden');if(text)text.textContent=`Encounter: ${description}. Use the command input: attack, attack [target], defend, flee, heal, or multiple strike.`;document.querySelectorAll('.dir-btn').forEach(button=>{button.disabled=true;button.setAttribute('aria-disabled','true');});const input=document.getElementById('cmd-input');if(input){input.disabled=true;input.setAttribute('aria-busy','true');}};
        Game.finishCommandCombat=function(){const status=document.getElementById('combat-status');status?.classList.add('hidden');const input=document.getElementById('cmd-input');if(input){input.disabled=false;input.removeAttribute('aria-busy');setTimeout(()=>input.focus(),0);}this.updateDirectionButtons?.(WorldData.locations[this.state.location]?.exits||{});};
        ...
        const previousCast=Game.castSpell.bind(Game);Game.castSpell=function(name){const key=String(name||'').toLowerCase().trim();if(HEALING_NAMES.some(n=>key===n||key==='healing spell'))return SpellSystem.castHealing(this);if(['multi strike','multiple strike','multiple strike spell'].includes(key))return SpellSystem.castMultipleStrike(this);if(['alohomora','opening doors','opening door','opening doors spell'].includes(key))return SpellSystem.castOpeningDoors(this);return previousCast(name);};
        ...
        const oldStart=Game.startCombat.bind(Game);Game.startCombat=function(name,queued=false){const add=this.addNarrative;if(!queued)this.addNarrative=function(text,type){if(/^A group of \d+ monsters surrounds you!$/.test(String(text))||/^You encounter (?:\d+ monsters?:|[^.]+\.$)/.test(String(text)))return;return add.call(this,text,type);};try{oldStart(name,queued);}finally{this.addNarrative=add;}if(!this.state.inCombat||!this.state.enemy)return;
            ...
            this.updateEnemyGroupHUD?.();const promise=this.emitGameEvent(message,'combat',{critical:true,eventId:`${this.state.battleSummary?.id||Date.now()}:encounter`});Promise.resolve(promise).finally(()=>{const input=document.getElementById('cmd-input');if(input&&this.state.inCombat){input.disabled=false;input.removeAttribute('aria-busy');input.focus();}});
        };
        ...
        const oldEnemyHud=Game.updateEnemyHUD.bind(Game);Game.updateEnemyHUD=function(){oldEnemyHud();const text=document.getElementById('combat-status-text');if(text&&this.state.inCombat&&this.state.enemy)text.textContent=`Current target: ${this.state.enemy.name}, ${Math.max(0,this.state.enemy.hp)} health. Use attack, defend, flee, heal, or multiple strike.`;};
    ...
    applyGamePatches();
    if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>{applyGamePatches();Mode.init();},{once:true});else Mode.init();
})();
